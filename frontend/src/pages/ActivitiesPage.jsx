...
// KOREKSI: Gunakan instance api buatan Anda, jangan gunakan axios mentah lagi!
...
export default function ActivitiesPage() {
  ...
  // ==============================================================================
  // --- PERBAIKAN SEGMEN: SINKRONISASI GOOGLE FIT KE FORMULIR ---
  // ==============================================================================
  ...
  const totalSteps = activities.reduce(
    (total, item) => total + Number(item.steps || 0),
    0
  );

  const totalCalories = activities.reduce(
    (total, item) => total + Number(item.calories_burned || 0),
    0
  );
      ...
      <div className="p-4 sm:p-6 pb-24 md:pb-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          ...
          <button
            ...
            className="bg-blue-600 text-white px-4 py-2.5 rounded-xl shadow-md hover:bg-blue-700 transition disabled:bg-gray-400 font-semibold text-sm sm:text-base text-center cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-8">
          <div className="bg-white p-5 sm:p-6 rounded-2xl shadow border border-gray-50">
            <h2 className="text-gray-500 text-sm sm:text-base font-medium">Total Steps</h2>
            <p className="text-2xl sm:text-3xl font-bold mt-1 text-gray-900">{totalSteps.toLocaleString()}</p>
          </div>
          <div className="bg-white p-5 sm:p-6 rounded-2xl shadow border border-gray-50">
            <h2 className="text-gray-500 text-sm sm:text-base font-medium">Total Calories Burned</h2>
            <p className="text-2xl sm:text-3xl font-bold mt-1 text-gray-900">{totalCalories.toLocaleString()} kcal</p>
          </div>
        </div>

        {/* Form */}
        <div className="bg-white p-5 sm:p-6 rounded-2xl shadow mb-8 border border-gray-50">
          <h2 className="text-xl sm:text-2xl font-bold mb-4 text-gray-800">Add Activity</h2>
          {message && (
              ...
              <div key={activity.id} className="bg-white p-4 rounded-2xl shadow border border-gray-100 flex flex-col gap-2">
                <div className="flex justify-between items-start">
                  ...
                  <button
                    ...
                    className="bg-red-50 text-red-500 hover:bg-red-500 hover:text-white px-3 py-1.5 rounded-xl text-xs font-semibold transition cursor-pointer"
                  >
                    Delete
                  </button>
                </div>
        ...
        <div className="hidden sm:block bg-white rounded-2xl shadow overflow-hidden border border-gray-100">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="p-4 text-left text-sm font-semibold text-gray-600">Activity</th>
                ...
                <th className="p-4 text-left text-sm font-semibold text-gray-600">Steps</th>
            ...
            <tbody>
              {activities.map((activity) => (
                <tr key={activity.id} className="border-t border-gray-100 hover:bg-gray-50/50 transition-colors">
                  <td className="p-4 text-sm font-medium text-gray-800">{activity.activity_type}</td>
                  <td className="p-4 text-sm text-gray-600">{activity.duration_minutes} min</td>
                  <td className="p-4 text-sm text-gray-600">{activity.calories_burned}</td>
                  <td className="p-4 text-sm text-gray-600">{activity.steps}</td>
                  <td className="p-4 text-sm text-gray-600">{activity.activity_date}</td>
                  <td className="p-4 text-sm text-gray-500 max-w-xs truncate">{activity.notes || "-"}</td>
                  <td className="p-4">
                    <button
                      onClick={() => handleDelete(activity.id)}
                      className="bg-red-500 text-white px-3 py-1.5 rounded-xl text-xs font-semibold hover:bg-red-600 transition cursor-pointer"
                    >
                      Delete
                    </button>
                  </td>
